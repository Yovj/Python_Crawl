...
def get_ZoneSingers(zone):
    ...
    for page in range(1,page_count+1):
      ...
      'notice=0&platform=yqq.json&needNewCode=0&data={}'.format(parse.quote(data))
        ...
        time.sleep(2)
        Thread.submit(get_ListSong,singer_of_page)
        sin += 80
...
# 给定歌手列表，拿到该分页下所有歌手的所有歌曲
def get_ListSong(singers):
...
def get_Song(singer):
    ...
    while(count <=5):
        ...
        try:
            ...
            print("get_Song_of {}....timeout.....{}".format(singer,count))
            count += 1
            if count == 6:
                print("get_Song_of {}....dead!!!".format(singer))
...
def spider():
    ...
    with ProcessPoolExecutor(max_workers=4) as executor:
        for i in range(1,3): # A-Z
            time.sleep(1)
            executor.submit(get_ZoneSingers,i)
    return
